...
function PostsSavePage() {
  ...
  useEffect(function () {
    getUserByEmail(user?.primaryEmailAddress.emailAddress).then((res) => setUserId(res.id));

    getAllSaves(userId).then((res) => setPostsSaved(res))
  ...
  return (
    <div className='px-[20px] lg:ms-[-200px] lg:col-span-2 relative'>
      <div className='flex items-center my-[50px]'>
        ...
        <h1 className='text-4xl lg:text-5xl pe-[20px] py-[10px] lg:py-[40px]'>Saved Posts</h1>
      </div>
      <div className='flex-col justify-center items-center lg:grid lg:grid-cols-4 lg:gap-[30px]'>
        {postsSaved.map((post) => <PostSaveContainer key={post.posts.id} post={post.posts} />)}
        {postsSaved.length === 0 && <p className='absolute top-[50%] left-[50%] translate-X-[-50%] translate-Y-[-50%] text-2xl font-bold'>No Posts Saved !!!</p>}
      </div>
    </div>
  )
}
