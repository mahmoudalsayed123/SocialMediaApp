...
function Like({ postUserId }) {
    ...
    return (
        <div onClick={handleAddLike} className='cursor-pointer transition-all duration-300 flex justify-center items-center'>
            {isClicked ? <FaHeart className='text-red-600 text-3xl' /> : <FaRegHeart className='text-3xl' />}
        </div>
    )
}
